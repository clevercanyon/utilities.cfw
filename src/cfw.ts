...
export const serviceBindingRequest = async (rcData: StdRequestContextData, requestInfo: $type.cfw.RequestInfo, requestInit?: $type.cfw.RequestInit): Promise<$type.cfw.Request> => {
    ...
    const userIP = { value: await $user.ip(parentRequest) },
        ...
        defaultIP = '8381:0387:2041:7bf4:3b0f:8f67:c52b:db57'; // Random IPv6.

    if (!userIP.value && (rcData as RequestContextData).scheduledEvent?.cron) {
        // Service binding requests generated by scheduled CRON events get a default IP.
        userIP.value = defaultIP; // Simply to avoid it being empty.

        // Now we do empty IP geolocation data, which is no longer accurate.
        for (const key of Object.keys(userIPGeoData)) {
            (userIPGeoData as $type.StrKeyable<typeof userIPGeoData>)[key] = '';
        }
    }
    headers.set('x-real-ip', userIP.value);
    headers.set('cf-connecting-ip', userIP.value);
